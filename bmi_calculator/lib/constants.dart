import 'package:flutter/material.dart';

const kLabelStyle = TextStyle(fontSize: 25.0);
const kSizedBoxHeight = 10.0;
const kIconSize = 80.0;
const kBottomButtonHeight = 100.0;
const Color kDefaultCardColor = Color(0xff111428);
const Color kActiveCardColor = Color(0xff1D1F33);
const kBottomButtonColor = Color(0xffFB2767);
const TextStyle kNumberStyle =
    TextStyle(fontSize: 50.0, fontWeight: FontWeight.w900);
const Color kSmallButtonFillColor = Color(0xff1C1F32);
const kSmallButtonElevation = 40.0;
